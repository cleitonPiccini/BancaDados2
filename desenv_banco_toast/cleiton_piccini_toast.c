...
typedef struct HeaderToast{
    int espacoLivre;
    int next;
}Htoast;

typedef struct ElemToast{
    int page;
    int inicio;
    int final;
}toastElement;
...
char* creatWord(toastElement e){
    char *a;
    char aux [5];
    a = malloc(17*sizeof(char));
    strcat(a,"*");
    sprintf(aux, "%i", e.page);
    strcat(a, aux);
    strcat(a , "&");
    sprintf(aux, "%i", e.inicio);
    strcat(a, aux);
    strcat(a , "&");
    sprintf(aux, "%i", e.final);
    strcat(a, aux);
    strcat(a , "&");
    //printf("%s", a);
    return a;
...
void createTable(char *sql) {
	...
	char tableName[500]; // define nome da tabela com 500 caracteres
  	char pageName[250]; // define nome da página com 500 caracteres
...
void buildHeader(char *sql, char *tableName, int qtdPages) {
    ...
    int fieldSize, i = 0, pageToast =0; //tamanho do campo e variavel auxiliar

    char sqlCopy[1000], pageName[250];  
    ...
    fwrite(&pageToast, sizeof(int), 1, headerPage);
    //fwrite(&pageToast, sizeof(int), 1, headerPage);
...
//Cria um novo Toast
void newToast(char *tableName, int numPage){
    Htoast toast;
    toast.espacoLivre = 8184;
    toast.next = 8;
    char pageName[250];
    snprintf(pageName, sizeof(pageName), "%s/toast%d.dat", tableName, numPage);
    FILE *page = fopen(pageName, "wb");
    fwrite(&toast.espacoLivre , sizeof(int), 1, page);
    fwrite(&toast.next, sizeof(int), 1, page);
    if(page == NULL) {
            printf("Failed to create page Toast\n");
            return;
        }else{
             printf("Toast %s already created\n", tableName);
        }
   fclose(page);
}

//Insert do toast.
char* InsertToast(char *tableName, char *token){
    char pageName[250];
    char *caminho = (char*) malloc(17*sizeof(char));
    int itens, pageToast;
    Htoast toast;
    toastElement item;
    ...
    fread(&itens, sizeof(int), 1, headerPage);
    fread(&pageToast,sizeof(int), 1, headerPage); 
    fclose(headerPage);
    if (pageToast == 0)
    {
        newToast(tableName, 1);
        pageToast++;
    }
    snprintf(pageName, sizeof(pageName), "%s/toast%d.dat", tableName, pageToast); 
    FILE *pageT = fopen(pageName, "rb+");
    fread(&toast.espacoLivre, sizeof(int), 1, pageT); 
    fread(&toast.next, sizeof(int), 1, pageT);
   // printf("itenms %d, pageToast %d\n",itens, pageToast );
    //printf(" espaco livre %d, next %d\n", toast.espacoLivre, toast.next);
    if (toast.espacoLivre < strlen(token))
    {
	   newToast(tableName, (pageToast+1));
       pageToast++;
       fclose(pageT);
       snprintf(pageName, sizeof(pageName), "%s/toast%d.dat", tableName, pageToast);
       /*FILE */pageT = fopen(pageName, "rb+");
       fseek(pageT, 0, SEEK_SET);
       fread(&toast.espacoLivre, sizeof(int), 1, pageT); 
       fread(&toast.next, sizeof(int), 1, pageT);
    }
    item.page = pageToast;
    item.inicio = toast.next;
    item.final = item.inicio + strlen(token) +1;
    fseek(pageT, toast.next, SEEK_SET);
    fwrite(token, strlen(token), 1, pageT);
    fwrite("#", 1, 1, pageT);
    caminho = creatWord(item);
    toast.next += strlen(token)+1;
    toast.espacoLivre = toast.espacoLivre - strlen(token)+1;
    fseek(pageT, 0,  SEEK_SET);
    fwrite(&toast.espacoLivre, sizeof(int), 1, pageT);
    fwrite(&toast.next, sizeof(int), 1, pageT);
    fclose(pageT);
    snprintf(pageName, sizeof(pageName), "%s/header.dat", tableName); 
    FILE *Page = fopen(pageName, "rb+"); 
    //fwrite(&i, sizeof(int), 1, headerPage);
    //fseek(Page, 0, SEEK_SET);
    fwrite(&itens, sizeof(int), 1, Page);
    fwrite(&pageToast, sizeof(int), 1, Page);
    fclose(Page);
    return caminho;
...
void insertInto(char *sql, int numPage) { 
	char sqlCopy[1000], *token, tableName[500], pageName[250], attrSql[1000], attrSqlCopy[1000], *caminho;
    char endVarchar = '$', special = ' ', endChar = '\0';
    int insertSize = 0, qtdFields, nextItem, intVar, nextPage, qtdEndChar = 0, pageToast;
    int i = 0;
    ...
    for(int i = 0; i < qtdFields; i++) {
        ...
        //printf("name %s, size %d, tipe %c\n", attributes[i].name, attributes[i].size, attributes[i].type );
    }
    ...
    if(head.memFree > insertSize) {
        ...
        caminho = (char*) malloc (17*sizeof(char));
            ...
            // char
      		if(attributes[i].type == 'C') {
                if (attributes[i].size <2048)
                {
                    ...
                    caminho = InsertToast(tableName, token);
                    fwrite(caminho, strlen(caminho), 1, page);
                }
                caminho = InsertToast(tableName, token);
                    fwrite(caminho, strlen(caminho), 1, page);
            }  
...
//Imprime o conteudo do toast.
void printToast (char tableName[50], int numPage, char endToast[17]){
	char *pageName;
    char pageBusca[600];
    char *token, copyEnd[17];
    char start[17], end[17];
    int intStart, intEnd, i = 0;

	memset(copyEnd, '\0', sizeof(copyEnd));
	memset (start , '\0', sizeof (start));
	memset (end , '\0', sizeof (end));
	
    strcpy(copyEnd, endToast);
    token = strtok(copyEnd, "&");
    

  while(token != NULL) { // procura o inicio e fim dentro do token.
        if (i == 1) strcpy(start, token);
        if (i == 2) strcpy(end, token);
        token = strtok(NULL, "&");
        i++;
    }
	...
	snprintf(pageBusca, sizeof(pageBusca), "%s/toast%d.dat",tableName, numPage ); //define o caminho da pagina de determinada tabela
	
    FILE *page = fopen(pageBusca, "rb+"); // abre a pagina do toast como leitura e escrita binária
	fseek(page, (intStart), SEEK_SET);
	pageName = (char*) malloc(sizeof(char)*(intEnd - intStart));
	fread(pageName, sizeof(char), (intEnd - intStart -1) , page);
	printf(" %s",pageName);
 

	free(pageName); // libera a memoria alocada.
    fclose(page); // fecha a página
}
...
void selectFrom(char *sql, int numPage) {
    char tableName[50], sqlCopy[1000], pageName[250], *token, charInFile, special = ' ';
    int i = 0, qtdFields = 0, moveItem = 0, intInFile, stopChar, pageToast;
    char endToast[17];
	header head;
    item readItem;
    attribute attributes[64];
    ...
    fread(&pageToast, sizeof(int), 1, headerPage);
	//printf("headerpage do fields %d\n", qtdFields);
	//printf("headerpage do toast %d\n", pageToast);
          	...
          	if(attributes[j].type == 'C') { 
				...
				if(attributes[j].size < 2048){
	                ...
	                for(int k = 0; k < attributes[j].size; k++) { //laço para percorer o registro do char
	              	...
	              	fread(&endToast, sizeof(char), 17, page);
	              	printToast(tableName, numPage, endToast);
              	}
...
void deleteFrom(char *sql, int numPage) {
    char tableName[50], sqlCopy[1000], pageName[600], *token, charInFile, attributeName[20], special = ' ';
    int i = 0, k = 0, qtdFields = 0, moveItem = 0, intInFile, stopChar, delete = 0, inputDelete, count = 0;
